...
/obj/effect/particle_effect/smoke/proc/smoke_atom(atom/A)
	if(iscarbon(A))
		...
		if(T.air)
			...
			for(var/obj/effect/hotspot/H in T)
				...
				if(G.toxins)
					G.nitrogen += (G.toxins)
					G.toxins = 0
		for(var/obj/machinery/atmospherics/unary/vent_pump/U in T)
...
/*
		for(var/obj/item/Item in T)
			Item.extinguish()
*/ //For when items become able to catch fire.
...
/obj/effect/particle_effect/smoke/chem/smoke_atom(atom/A)
	..()
