	...
	if(power_drained >= 100 && total_power_drained >= power_damage_threshhold)
		...
		if(ishuman(invoker))
			...
			for(var/X in H.bodyparts)
				var/obj/item/organ/BP = X
				if(ratvar_awakens || BP.status == ORGAN_ROBOT && total_power_drained < augument_damage_threshhold) //if ratvar is alive, it won't damage and will always heal augumented limbs
