...
/proc/ghost_follow_link(var/atom/target, var/atom/ghost)
	...
	if(isAI(target)) // AI core/eye follow links
		...
		. = "<a href='?src=[ghost.UID()];follow=\ref[A]'>core</a>"
		if(A.client && A.eyeobj) // No point following clientless AI eyes
			. += "|<a href='?src=[ghost.UID()];follow=\ref[A.eyeobj]'>eye</a>"
		return
	else if(istype(target, /mob/dead/observer))
		var/mob/dead/observer/O = target
		. = "<a href='?src=[ghost.UID()];follow=\ref[target]'>follow</a>"
		if(O.mind && O.mind.current)
			. += "|<a href='?src=[ghost.UID()];follow=\ref[O.mind.current]'>body</a>"
		return
	else
		return "<a href='?src=[ghost.UID()];follow=\ref[target]'>follow</a>"
