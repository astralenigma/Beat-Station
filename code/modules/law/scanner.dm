	...
	if( console && istype( O ))
		...
		if( console.incident && C.mob )
			if( console.incident.isInTrial( C.mob ) )
				to_chat(user, "<span class='alert'>That person already has a role in the trial! Clear them from the console first to change their role.</span>")
				return
			...
			if( console.incident.criminal )
				to_chat(user, "<span class='alert'>There is already a defendant! Clear them from the console to take their place.</span>")
			else
				console.incident.criminal = C.mob
				ping( "\The [src] pings, \"Defendant [C.mob] verified.\"" )
		else
			if( !console.incident )
				to_chat(user, "<span class='alert'>Console has no active trial!</span>")
			else if( !C.mob )
				to_chat(user, "<span class='alert'>ID is not tied to a NanoTrasen Employee!</span>")
		return
